...
double physics::get_sound_speed(double density, double pressure) {
	// done
	double cs = sqrt(adiabatic_index*pressure/density);
	return cs;
}
...
void physics::get_lambda_min_max(double &lambda_min, double &lambda_max, const fluid_cell &fluid_left_cell, const fluid_cell &fluid_right_cell,
	...
	double density_left = fluid_left_cell.fluid_data[index_density];  // done
	double density_right = fluid_right_cell.fluid_data[index_density]; // done

	double v_parallel_left = fluid_left_cell.fluid_data[index_velocity_parallel];
	double v_parallel_right = fluid_right_cell.fluid_data[index_velocity_parallel]; // done

	double pressure_left = get_pressure(fluid_left_cell);
	double pressure_right = get_pressure(fluid_right_cell); // done

	double sound_speed_left = get_sound_speed(density_left, pressure_left);
	double sound_speed_right = get_sound_speed(density_right, pressure_left); // done

	lambda_max = std::max(v_parallel_left + sound_speed_left, v_parallel_right + sound_speed_right);
	lambda_min = std::min(v_parallel_left - sound_speed_left, v_parallel_right - sound_speed_right); // done
}
